 ...
 */
export default class WebhooksProcessor {
    ...
    constructor(_client) {
        this[client] = _client;
        /** @type {tranzak_node.webhook.WebhooksEventEmitter} */ this.events
    ...
    async process(data) {
        ...
        if (
            !/^((REQUEST\.((COMPLETED)|(CANCELED)))|(TRANSFER\.COMPLETED))$/
                .test(data.eventType)
        ) {
        ...
        const getTransaction = async () => {
            ...
            switch (data.eventType) {
                ...
                case 'REQUEST.COMPLETED':
                    return await this[client].payment.collection.simple.find({ requestId: data.resource.requestId });

                case 'TRANSFER.COMPLETED':
                    return await this[client].payment.transfer.simple.find({ transferId: data.resource.transferId });
            }
        }
        ...
        try {
            ...
            if (
                (
                    /COMPLETED/.test(data.eventType) && transaction.data.status !== 'SUCCESSFUL'
                )
                ||
                (
                    /CANCELED/.test(data.eventType) && (
                        (transaction.data.status !== 'CANCELLED') && (transaction.data.status !== 'CANCELLED_BY_PAYER')
                    )
                )
